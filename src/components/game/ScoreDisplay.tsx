...
export function ScoreDisplay({ score, streak, bestStreak }: ScoreDisplayProps) {
  ...
  useEffect(() => {
    ...
    if (delta > 0) {
      setScoreDelta(delta);
      const timer = setTimeout(() => setScoreDelta(0), 1800);
      return () => clearTimeout(timer);
    }
  }, [score]);
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2 sm:gap-3 flex-wrap justify-center">
        ...
        <motion.div
          className="relative flex items-center gap-1.5 sm:gap-2 bg-gradient-to-r from-amber-50 to-yellow-50 px-4 py-2 sm:px-5 sm:py-2.5 rounded-full shadow-lg border-2 border-amber-300"
          whileHover={{ scale: 1.05 }}
          key={`score-${score}`}
          initial={score > 0 ? { scale: 1.1 } : false}
          animate={{ scale: 1 }}
          ...
          <AnimatePresence>
            {scoreDelta > 0 && (
              <motion.span
                ...
                className="absolute -top-1 -right-2 text-sm sm:text-base font-bold text-green-600 pointer-events-none"
              >
                +{scoreDelta}
              </motion.span>
            )}
        ...
        {/* Current Streak */}
        <motion.div
          className={`flex items-center gap-1.5 sm:gap-2 bg-white px-3 py-2 sm:px-4 sm:py-2.5 rounded-full shadow-md border-2 ${tier.border} transition-colors`}
          whileHover={{ scale: 1.05 }}
          key={streak}
        ...
        {/* Best Streak */}
        <motion.div
          className="flex items-center gap-1.5 sm:gap-2 bg-gradient-to-r from-primary-500 to-primary-600 px-3 py-2 sm:px-4 sm:py-2.5 rounded-full shadow-md text-white"
          whileHover={{ scale: 1.05 }}
        >
